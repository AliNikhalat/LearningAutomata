...
s_vdhla_2 = SymmetricVariableDepthHybrid(
    action_number, 4, reward_rate_2, penalty_rate_2)
...
for i in range(iteration_number):
    ...
    s_vdhla_1.visualization_calculations()

    favorable_vdhla_action_probability_1.append(
        s_vdhla_1.get_action_selection_status(0)[-1] / (i + 1))

    # VDHLA_2
    chosen_action_vdhla_2 = 0
    if i != 0:
        chosen_action_vdhla_2 = s_vdhla_2.choose_action()
    else:
        chosen_action_vdhla_2 = s_vdhla_2.choose_random_action()

    evaluated_action_vdhla_2 = environment.evaluate_action(
        chosen_action_vdhla_2)
    s_vdhla_2.receive_environment_signal(evaluated_action_vdhla_2)
    s_vdhla_2.visualization_calculations()

    favorable_vdhla_action_probability_2.append(
        s_vdhla_2.get_action_selection_status(0)[-1] / (i + 1))
...
plt.plot(x_values, favorable_vdhla_action_probability_1,
         color='r', label='Symmetric VDHLA(Ex1.2.2.8)')
plt.plot(x_values, favorable_vsla_action_probability_1,
         color='r', label='VSLA(Ex1.2.2.8)', linestyle='dashed')

plt.plot(x_values, favorable_vdhla_action_probability_2,
         color='b', label='Symmetric VDHLA(Ex1.2.2.9)')
plt.plot(x_values, favorable_vsla_action_probability_2,
         color='b', label='VSLA(Ex1.2.2.9)', linestyle='dashed')

plt.title('VSLA Comparison-Ex1.2.2.8,Ex1.2.2.9')
